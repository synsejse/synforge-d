...
export type RefreshOverlayTone = "running" | "success" | "error";

export function refreshTitle(state: RefreshAllPackagesProgressView["state"]): string {
  if (state === "running") return "Refreshing enabled packages";
  if (state === "failed") return "Refresh all failed";
  return "Refresh all complete";
}

export function refreshTone(
...
export type RefreshAllOverlayProps = {
  open: boolean;
...
export default function RefreshAllOverlay({
  open,
  operation,
  refreshing,
  onClose,
}: RefreshAllOverlayProps) {
  const title = operation ? refreshTitle(operation.state) : "Refreshing enabled packages";
  const tone: RefreshOverlayTone = operation ? refreshTone(operation.state) : "running";
  ...
  const summary = operation
    ...
    : "Preparing…";
  // Only surface the message line when it's not redundant with the stats
  // (e.g. an error string while failed, or "collecting enabled packages"
  // pre-roll). Plain progress noise gets suppressed.
  const message =
    operation?.message && operation.state !== "completed" ? operation.message : null;
